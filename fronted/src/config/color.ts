...
export function getBrandColor(type: string, colorList: ColorSeries): ColorToken {
  ...
  return colorList[name || "DEFAULT"] || colorList["DEFAULT"] || {}
}

export function getColorList(colorArray: Array<ColorToken>): Array<string> {
  const pureColorList: string[] = []
  colorArray.map(colorToken => Object.keys(colorToken).map(key => {
    const value = colorToken[key]
    if (typeof value === 'string') {
      pureColorList.push(value)
    }
  }))
...
export function generateColorMap(theme: string, colorPalette: Array<string>, mode: "light" | "dark") {
  ...
  if (isDarkMode) {
    colorPalette = colorPalette.reverse().map(color => {
      ...
      return hslToHex(h, Math.max(0, s - 4), l)
    })
    brandColorIdx = 5
